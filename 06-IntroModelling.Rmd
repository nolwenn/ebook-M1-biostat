...
The most common logistic regression models a binary outcome (Dead/Alive, yes/no). Multinomial logistic regression can model scenarios where there are more than two possible discrete outcomes. 
...
When coding the outcome variable 0 and 1 we can still represent what is happening in association with a covariate such as age. 
```{r, echo=FALSE}
...
```{r plot ckd, out.height="350px", out.width="350px", message=F,warning=FALSE}
...
- $Y = \frac{1}{1 + e^{-(\beta_0 + \beta_1X)}}$
 ...
 \centerline{\includegraphics[height=2in]{fig/logReg5.png}}

- $\beta_0$ and $\beta_1$ give the shape of the curve

We note the the $\beta_0 + \beta_1X$ part looks really similar a  linear model. In fact, to estimate those coefficient a mathematical trick is to transform the above equation in order to get to a linear form.
...
The coefficient is positively link to the disease. When you increase in age you increase your risk of having disease. To compute the OR, we can use the $exp(0.039)$ but for a nicest output with confidence interval we can use the *epiDisplay* library and its *logistic.display()* function.
